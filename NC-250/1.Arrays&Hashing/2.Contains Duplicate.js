/**
 * 217. Contains Duplicate
 * 
 * Given an integer array nums, return true if any value appears at least twice in the array, 
 * and return false if every element is distinct.
 */

const nums1 = [1, 2, 3, 4]
const nums2 = [1, 2, 3, 3]

// Hash Set Length
// Time complexity: O(n) where n is the length of array nums (creating Set takes O(n) time)
// Space complexity: O(n) where n is the length of array nums (Set stores up to n elements)
function hasDuplicate4(nums) {
    return new Set(nums).size < nums.length
}

// Hash Set  
// Time complexity: O(n) where n is the length of array nums (single pass through array)
// Space complexity: O(n) where n is the length of array nums (Set stores up to n elements)
function hasDuplicate5(nums) {
    let seen = new Set()
    for(const num of nums) {
        if(seen.has(num)) {
            return true
        }
        seen.add(num)
    }
    return false
}

// Hash Map
// Time complexity: O(n) where n is the length of array nums (single pass through array)
// Space complexity: O(n) where n is the length of array nums (up to n elements stored in map)
function hasDuplicate3(nums) {
    let map = new Map()
    for(const key of nums) {
        map[key] = (map[key] | 0) + 1
        if(map[key] > 1) {
            return true
        }
    }
    return false
}

// Sorting
// Time complexity: O(n log n) where n is the length of array nums (sorting the array)
// Space complexity: O(1) or O(n) where n is the length of array nums (depending on the sorting algorithm)
function hasDuplicate2(nums) {
    const sortedArr = nums.slice().sort((a, b) => a - b)
    const length = sortedArr.length
    for(let i = 0; i < length - 1; i++) {
        if (sortedArr[i] === sortedArr[i+1]) {
            return true
        }
    }
    return false
}

// Brute Force
// Time complexity: O(n^2) where n is the length of array nums (nested loops comparing each element)
// Space complexity: O(1) (no extra memory required)
function hasDuplicate1(nums) {
    let hasDuplicate = false
    const len = nums.length
    for(let i = 0; i < len - 1; i++) {
        for(let j = i + 1; j < len; j++) {
            if(nums[i] === nums[j]) {
                hasDuplicate = true
                break
            }
        }
        if(hasDuplicate) {
            break;
        }
    }
    return hasDuplicate
}

console.log("Hash Set Length:")
console.log(hasDuplicate4(nums2))

console.log("\nHash Set:")
console.log(hasDuplicate5(nums2))

console.log("\nHash Map:")
console.log(hasDuplicate3(nums2))

console.log("\nSorting:")
console.log(hasDuplicate2(nums2))

console.log("\nBrute Force:")
console.log(hasDuplicate1(nums2))